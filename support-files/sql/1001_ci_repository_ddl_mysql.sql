...
-- ----------------------------
-- Table structure for T_REPOSITORY_CODE_GITEE
-- ----------------------------

CREATE TABLE IF NOT EXISTS `T_REPOSITORY_CODE_GITEE`
(
    `REPOSITORY_ID` bigint(20) NOT NULL COMMENT '仓库ID',
    `PROJECT_NAME` varchar(255) NOT NULL COMMENT '项目名称',
    `USER_NAME` varchar(64) NOT NULL COMMENT '用户名称',
    `CREATED_TIME` timestamp NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '创建时间',
    `UPDATED_TIME` timestamp NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `CREDENTIAL_ID` varchar(64) NOT NULL COMMENT '凭据 ID',
    `AUTH_TYPE` varchar(8) DEFAULT NULL COMMENT '凭证类型',
    PRIMARY KEY (`REPOSITORY_ID`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='gitee代码库明细表';
